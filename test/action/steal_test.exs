defmodule Werewolf.Action.StealTest do
  ...
  describe "resolve/2" do
    ...
    test "when werewolf_thief alive, successfully steals from dead player", context do
      players = context[:additional_player_map]
    ...
    test "when werewolf_thief alive, successfully steals from player with multiple items, but one used",
         context do
      players = context[:additional_player_map]
      {:ok, player} = Player.add_action(players["werewolf_thief"], 1, Action.new(:steal, "witch"))
      players = put_in(players["werewolf_thief"], player)
      ...
      items = [
        %Item{type: :poison, remaining_uses: 0},
        %Item{type: :resurrection_scroll, remaining_uses: 1},
        %Item{type: :hammer, remaining_uses: 0}
      ]

      players = put_in(players["witch"].items, items)

      {:ok, players} = Action.Steal.resolve(players, 1)
      assert players["witch"].actions[1][:theft].type == :theft
      assert length(players["witch"].items) == 2
      assert length(players["werewolf_thief"].items) == 2
      assert players["werewolf_thief"].actions[1][:steal].result == :resurrection_scroll
    end
