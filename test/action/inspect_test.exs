defmodule Werewolf.Action.InspectTest do
  ...
  describe "resolve/2" do
    ...
    test "when detective alive, successfully inspects player", context do
      ...
      {:ok, players} = Action.Inspect.resolve(players, 1, %Options{})
    ...
    test "when detective alive, no role reveal on inspect, successfully inspects player",
         context do
      players = context[:player_map]
      {:ok, player} = Player.add_action(players["detective"], 1, Action.new(:inspect, "villager"))
      players = put_in(players["detective"], player)

      {:ok, players} = Action.Inspect.resolve(players, 1, %Options{reveal_role_on_inspect: false})
      assert players["detective"].actions[1][:inspect].result == :order
    end

    test "when detective alive, no role reveal on inspect, successfully inspects werewolf player",
         context do
      players = context[:player_map]
      {:ok, player} = Player.add_action(players["detective"], 1, Action.new(:inspect, "werewolf"))
      players = put_in(players["detective"], player)
      ...
      {:ok, players} = Action.Inspect.resolve(players, 1, %Options{reveal_role_on_inspect: false})
      assert players["detective"].actions[1][:inspect].result == :chaos
    end
