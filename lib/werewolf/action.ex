defmodule Werewolf.Action do
  ...
  def resolve_overrule_action(players, phase_number) do
    with {:ok, players_with_item} <-
           find_players_for_items(Map.values(players), [:scales_of_justice]),
         {:ok, player_and_valid_actions} <-
           find_actions_for_phase(players_with_item, players, phase_number, :overrule) do
      {:ok,
       Enum.reduce(player_and_valid_actions, players, fn {_player, action}, acc_players ->
         put_in(
           acc_players[action.target].alive,
